"""命令：
    python sand_client_mode.py install
    python sand_client_mode.py uninstall
    python sand_client_mode.py set-path <Cursor路径|auto>
"""
...
VIRTUAL_CARD_URL = "https://wap.roogoo.store/register?inviteCode=jk2bq3"

ANNOUNCEMENT = f"""
======================================================================
领取资格提示
使用前请确保当前 Cursor 账号已经获得 Sand 资格。
官方领取页面：{SAND_ONBOARDING_URL}
领取过程可能需要绑定银行卡；虚拟卡申请：{VIRTUAL_CARD_URL}
======================================================================
""".strip()
...
class PatchStats:
    ...
    @property
    def total(self) -> int:
        return self.is_glass + self.object_header + self.set_header + self.eligibility
...
class PatchStatus:
    ...
    def installed(self) -> bool:
        return self.client_markers + self.eligibility_markers > 0
...
def resolve_cursor_layout() -> CursorLayout:
    ...
    for source, values in _default_candidate_groups():
        ...
        if len(layouts) > 1:
            ...
            raise SandToolError(
                f"{source}检测到多个 Cursor 安装，请先设置路径：\n{options}\n"
                "命令：python sand_client_mode.py set-path <路径>"
            )
    raise SandToolError(
        "未检测到 Cursor 安装，请运行：\n"
        "python sand_client_mode.py set-path <Cursor.exe、Cursor.app 或 resources/app 路径>"
    )
...
def remove_patch_from_content(content: str) -> Tuple[str, RemoveStats]:
    ...
    next_content = client_re.sub(remove_client, content)
    ...
    next_content, stats.eligibility = eligibility_re.subn("", next_content)
    return next_content, stats
...
def inspect_status(layout: CursorLayout) -> PatchStatus:
    ...
    for target in layout.target_paths:
        ...
        if client_count + eligibility_count:
...
def install(layout: CursorLayout) -> int:
    print(f"检测到 Cursor {layout.version}：{layout.install_root}")
    print("安装会自动关闭 Cursor，请先保存尚未保存的内容。")
    closed = close_cursor(layout)
    if closed:
        print(f"已关闭 Cursor 进程：{closed} 个")
    ...
    if not plan:
        if before.installed:
            print(
                "Sand 客户端模式已经安装，"
                f"marker={before.client_markers + before.eligibility_markers}"
            )
            return 0
    ...
    def validate() -> None:
        ...
        if not status.installed or status.ide_matches != 0:
            raise SandToolError(
                "安装后状态校验失败："
    ...
    changed, backup_dir = _commit_plan(layout, plan, "install", validate)
    close_cursor(layout)
    status = inspect_status(layout)
    print("Sand 客户端模式安装成功，Cursor 保持关闭。")
    print(
        "变更统计："
        f"client-type={stats.is_glass + stats.object_header + stats.set_header}，"
        f"资格规则={stats.eligibility}，接管裸 sand={stats.adopted_sand}，"
        f"文件={len(changed)}，marker={status.client_markers + status.eligibility_markers}"
    )
    print(f"事务备份：{backup_dir}")
    print("请手动重新启动 Cursor 使配置生效。")
    return 0


def uninstall(layout: CursorLayout) -> int:
    print(f"检测到 Cursor {layout.version}：{layout.install_root}")
    print("卸载会自动关闭 Cursor，请先保存尚未保存的内容。")
    closed = close_cursor(layout)
    if closed:
        print(f"已关闭 Cursor 进程：{closed} 个")

    plan, stats = _build_uninstall_plan(layout)
    if not plan:
        print("未检测到本脚本管理的 Sand marker，无需卸载。")
        return 0
    ...
    changed, backup_dir = _commit_plan(layout, plan, "uninstall", validate)
    close_cursor(layout)
    print("Sand 客户端模式卸载成功，Cursor 保持关闭。")
    print(
        f"回收统计：client-type={stats.client_type}，资格规则={stats.eligibility}，"
        f"文件={len(changed)}"
    )
    print(f"事务备份：{backup_dir}")
    print("请手动重新启动 Cursor。")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  python sand_client_mode.py install\n"
            "  python sand_client_mode.py uninstall\n"
            "  python sand_client_mode.py set-path \"E:\\Development\\IDE\\cursor\"\n"
            "  python3 sand_client_mode.py set-path /Applications/Cursor.app\n"
            "  python sand_client_mode.py set-path auto"
        ),
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        if args.command == "set-path":
            layout = save_cursor_path(args.path)
            if layout is None:
                print("已清除自定义 Cursor 路径，后续将自动检测。")
            else:
                print("Cursor 路径设置成功：")
                print(f"  安装目录：{layout.install_root}")
                print(f"  appRoot：{layout.app_root}")
                print(f"  版本：{layout.version}")
                print(f"  可识别目标：{len(layout.target_paths)} 个")
            return 0
        ...
        print(f"错误：没有写入权限：{exc}", file=sys.stderr)
