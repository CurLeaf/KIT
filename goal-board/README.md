...
静态页，无构建。鼓轮 + 准星带展示当日目标。每天的 json 数据都是不同的 json，并且今日只用当前的 json。
...
`prefers-reduced-motion: reduce` 时不自动滚。
